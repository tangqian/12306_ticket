...
public class PassengerPanelManager {
    ...
    public static void init(JFrame frame) {
        if (!isInited) {
            ...
            parent.setBounds(10, 76, 650, 260);
            parent.setBorder(new TitledBorder(ResManager.getText("ticket.panel.passengerinfo")));
            frame.getContentPane().add(parent);
            
            JScrollPane scrollPane = new JScrollPane();
            scrollPane.setBounds(10, 20, 630, 185);
    ...
    public static List<ContacterInfo> getPassenger() {
        int rowCount = tableModel.getRowCount();
        List<ContacterInfo> lists = new ArrayList<ContacterInfo>();
        if (rowCount > 0) {
            for (int i = 0; i < rowCount; i++) {
                ...
                if(StringUtils.isBlank(name)){
                    TicketMainFrame.remind("第" + i + "行，用户姓名未填，请检查");
                    continue;
                }else if(StringUtils.isBlank(cardNo) ){
                    TicketMainFrame.remind("第" + i + "行，身份证号未填，请检查");
                    continue;
                }else{
                    ContacterInfo info = new ContacterInfo(name, cardNo, mobile);
                    lists.add(info);
                }
            }
        }
