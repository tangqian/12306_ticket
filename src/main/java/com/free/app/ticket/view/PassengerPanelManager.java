			...
			for (ContacterInfo contacterInfo : selected) {
				if (table.getRowCount() >= 5) {
					...
					if (contacterInfo == null)
						continue;
					String[] rowValues = { contacterInfo.getPassenger_name(),
							contacterInfo.getPassenger_id_no(),
	...
	private static JPanel getOperatePanel(final DefaultTableModel tableModel) {
		...
		addButton.addActionListener(new ActionListener() {// 添加事件
					public void actionPerformed(ActionEvent e) {
						if (table.getRowCount() >= 5) {
							...
							String[] rowValues = { passengerName.getText(),
									cardNo.getText(), mobile.getText() };
							tableModel.addRow(rowValues); // 添加一行
						}
		...
		updateButton.addActionListener(new ActionListener() {// 添加事件
					public void actionPerformed(ActionEvent e) {
						...
						{
							// 修改指定的值：
							tableModel.setValueAt(passengerName.getText(),
									selectedRow, 0);
							tableModel.setValueAt(cardNo.getText(),
									selectedRow, 1);
							tableModel.setValueAt(mobile.getText(),
									selectedRow, 2);
						} else {
		...
		if (passengers != null && !passengers.isEmpty()) {
			for (PassengerData passenger : passengers) {
				if (table.getRowCount() >= 5) {
					...
					String[] rowValues = { passenger.getName(),
							passenger.getCardNo(), passenger.getMobile() };
					tableModel.addRow(rowValues); // 添加一行
				}
			}
		}
